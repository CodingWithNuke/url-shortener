<template>
  <v-dialog v-model="internalValue" width="500">
    <v-form @submit.prevent="onSubmit">
      <v-card>
        <v-card-title>Edit Shortened URL</v-card-title>
...
export default {
  ...
  methods: {
    async onSubmit() {
      ...
      const { _id: id } = this.editUrl;

      try {
        await this.$store.dispatch("updateURL", {
          id,
          code,
          url: { url, slug }
        });
